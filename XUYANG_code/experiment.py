...
def train_dqn(instances, cfg, out_dir):
    ...
    for ep in range(1, cfg.episodes + 1):
        ...
        agent.decay_epsilon()  # decay once per episode for predictable exploration schedule
        if cfg.lr_decay < 1.0:
...
def visualize_model(algorithm, model_path, instance, out_dir):
    """Run a trained model on one instance and generate Gantt + fatigue charts."""
...
def main():
    ...
    dqn_path = OUTPUT_DIR / "dqn" / "dqn_model.pt"
    ppo_path = OUTPUT_DIR / "ppo" / "ppo_model.pt"

    if TRAIN_DQN:
        dqn_path = train_dqn(train_instances, cfg, OUTPUT_DIR / "dqn")
    if TRAIN_PPO:
        ppo_path = train_ppo(train_instances, cfg, OUTPUT_DIR / "ppo")
    ...
    viz_instance = train_instances[0]  # visualize on the first training instance
